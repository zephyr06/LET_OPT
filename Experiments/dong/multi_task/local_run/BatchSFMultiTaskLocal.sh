...
perform_optimization() {
	...
	for file_index in $(seq $MinFileIndex $files_per_task $MaxFileIndex); do
		...
		echo "$ROOT_PATH/release/RunSingleFile/BatchRunSF.run --N $N --begin $file_index --end $end_index &"
		$ROOT_PATH/release/RunSingleFile/BatchRunSF.run --N $N --begin $file_index --end $end_index &
	done
