...
files_per_task_list=( 8 8 8 8 8 8 8 8 8 8)
...
perform_optimization() {
	...
	files_per_task=$2
	time_per_node=$3
	output_file_name=log/BatchDAMultiChain_${task_number}-$(date +"%Y%m%d%H%M%S").out
	echo "sbatch -J BatchDAMultiChain_${task_number} --nodes=1 --ntasks-per-node=$(((MaxFileIndex+files_per_task)/files_per_task)) --cpus-per-task=1 --time=${time_per_node}:0:0 --output=${output_file_name} SBatchDAMultiTaskMultiChains.sh $1 $MinFileIndex $files_per_task $MaxFileIndex"
	## test in local
	# bash SBatchDAMultiTask.sh $1 $MinFileIndex $files_per_task $MaxFileIndex
	## launc on ARC
	sbatch -J BatchDAMultiChain_${task_number} --nodes=1 --ntasks-per-node=$(((MaxFileIndex+files_per_task)/files_per_task)) --cpus-per-task=1 --time=${time_per_node}:0:0 --output=${output_file_name} SBatchDAMultiTaskMultiChains.sh $1 $MinFileIndex $files_per_task $MaxFileIndex
}
