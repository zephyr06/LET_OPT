...
class RTA_LL : public RTA_BASE {
 public:
  ...
  double ResponseTimeAnalysisWarm_util_nece(double beginTime,
                                            const Task &taskCurr,
    ...
    if (isnan(taskCurr.executionTime) || isnan(beginTime)) {
      std::cout << Color::red << "Nan executionTime detected" << Color::def
                << std::endl;
      throw "Nan";
    }
    ...
    while (not stop_flag) {
      ...
      for (int i = 0; i < N; i++)
        responseTime += ceil(responseTimeBefore / double(periodHigh[i])) *
                        executionTimeHigh[i];
  ...
  double ResponseTimeAnalysis(const Task &taskCurr,
    ...
    for (auto &task : tasksHighPriority) executionTimeAll += task.executionTime;
...
inline void PrintRta(const std::vector<int> &rta) {
  std::cout << "RTA: ";
  for (int x : rta) std::cout << x << ", ";
  std::cout << "\n";
}
