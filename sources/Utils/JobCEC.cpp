...
void PrintSchedule(const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
    ...
    for (auto time_job_pair : timeJobVector) {
        ...
        std::cout << "T" << time_job_pair.second.taskId << "_"
                  << time_job_pair.second.jobId << " ("
...
LLint GetJobUniqueIdWithinHyperPeriod(
    ...
    if (jobCEC.jobId >= tasksInfo.sizeOfVariables[jobCEC.taskId]) {
        JobCEC jobWithinHyperPeriod(
            jobCEC.taskId,
            jobCEC.jobId % tasksInfo.sizeOfVariables[jobCEC.taskId]);
...
double GetHyperPeriodDiff(
    ...
    const RegularTaskSystem::TaskSetInfoDerived &tasksInfo) {
    int startHpIndex =
        jobStart.jobId / tasksInfo.sizeOfVariables[jobStart.taskId];
    int finishHpIndex =
