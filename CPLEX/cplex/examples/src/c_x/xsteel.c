                     ...
                     double *flatinv, double *flatsell, int buildbycolumn)
{
   ...
   /* If an error occurs, the status value indicates the reason for
      ...
      we can't call CPXXmsgstr if CPXXopenCPLEX failed.  */
   ...
   /* Now that the channels have the error message function, we can
      use CPXXmsgstr (via display) for errors. */
   ...
   *lpstat_p = lpstat;
   ...
   if ( status ) {
      display (cpxerror, "CPXXgetx failed to get Make solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
   ...
   if ( status ) {
      display (cpxerror, "CPXXgetx failed to get Inv solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
   ...
   status = CPXXgetx (env, lp, flatsell, 2*numprod*tweeks, 
                     3*numprod*tweeks-1);
   if ( status ) {
      display (cpxerror, "CPXXgetx failed to get Sell solution.  Status %d.\n",
              status);
      goto TERMINATE;
   }
