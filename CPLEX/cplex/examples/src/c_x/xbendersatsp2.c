...
/* Example xbendersatsp2.c solves a flow MILP model for an
   ...
   the function CPXXcallbackrejectcandidate.
...
static int
   ...
   init_user_data        (USERDATA *globaldata,
                          CPXDIM num_nodes, CPXDIM num_x_cols, int num_cores),
   init_user_localdata   (USERLOCALDATA *localdata,
                          CPXDIM num_nodes, CPXDIM num_x_cols);
...
main (int  argc, char *argv[])
{
   ...
   status = CPXXgetnumcores (env, &num_cores);
   if ( status ) {
      fprintf (stderr,
                      ...
                      CPXDIM num_nodes, CPXDIM num_x_cols)
{
   ...
   localdata->env = CPXXopenCPLEX (&status);
   ...
   status = CPXXsetintparam (localdata->env, CPXPARAM_Preprocessing_Reduce,
                             0);
   if ( status ) {
      fprintf(stderr,
   ...
   localdata->lp = CPXXcreateprob (localdata->env, &status,
                                       "atsp_worker.lp");
   ...
   for (k = 1; k < num_nodes; ++k) {
      for (i = 0; i < num_nodes; ++i) {
         for (j = 0; j < num_nodes; ++j) {
            ...
            status = CPXXnewcols (localdata->env, localdata->lp, 1,
   ...
   status = CPXXaddrows (localdata->env, localdata->lp,
...
free_user_localdata  (USERLOCALDATA *localdata)
{
   ...
   if ( localdata->lp != NULL ) {
      int status = CPXXfreeprob (localdata->env, &(localdata->lp) );
      if ( status ) {
         fprintf (stderr, "CPXXfreeprob failed, error code %d.\n", status);
      }
      else
   ...
   if ( localdata->env != NULL ) {
      int status = CPXXcloseCPLEX ( &(localdata->env) );
      if ( status ) {
                       ...
                       USERDATA *cbhandle)
{
   ...
   status = CPXXcallbacksetfunc (env, lp, where, benders_callback, cbhandle);
   if ( status )  {
      fprintf (stderr,
         "Error in CPXXcallbacksetfunc, status = %d.\n",
         status);
      goto TERMINATE;
   }
                   ...
                   CPXLONG contextid, void *cbhandle)
{
   ...
   CPXDIM k, nzcnt, cur_x_col, cur_v_col, cur_u_col, num_x_cols, num_nodes;
   CPXNNZ beg = 0;
   ...
   status = CPXXcallbackgetinfoint (context, CPXCALLBACKINFO_THREADID, &me);
   if ( status ) {
      fprintf (stderr,
               "Error in CPXXcallbackgetinfoint: status = %d\n", status);
      goto TERMINATE;
   }
   ...
   case CPX_CALLBACKCONTEXT_CANDIDATE:
      status = CPXXcallbackcandidateispoint (context, &point);
      if ( status ) {
         fprintf (stderr,
      ...
      status = CPXXcallbackgetcandidatepoint (context, localdata->x,
                                              0, num_x_cols - 1, NULL);
      if ( status ) {
         fprintf (stderr,
                  "Error in CPXXcallbackgetcandidatepoint: status = %d\n",
                  status);
         goto TERMINATE;
      }
      break;
   case CPX_CALLBACKCONTEXT_RELAXATION:
      status = CPXXcallbackgetrelaxationpoint (context, localdata->x,
                                          0, num_x_cols - 1, NULL);
      if ( status ) {
         fprintf (stderr,
                  "Error in CPXXcallbackgetrelaxationpoint: status = %d\n",
                  status);
         goto TERMINATE;
      }
      break;
   ...
   for (k = 1; k < num_nodes; ++k) {
      ...
      status = CPXXchgobj (localdata->env, localdata->lp,
                           num_x_cols,
                           localdata->indices,
                           localdata->x);
      if ( status ) {
   ...
   status = CPXXprimopt (localdata->env, localdata->lp);
   if ( status ) {
      fprintf (stderr, "Error in CPXXprimopt: status = %d\n", status);
      goto TERMINATE;
   }
   worker_lp_sol_stat = CPXXgetstat (localdata->env, localdata->lp);
   if ( worker_lp_sol_stat != CPX_STAT_UNBOUNDED)
      goto TERMINATE;

   /* Get the violated cut as an unbounded ray of the worker LP */

   status = CPXXgetray (localdata->env, localdata->lp, localdata->ray);
   if ( status ) {
   ...
   switch (contextid) {
   case CPX_CALLBACKCONTEXT_CANDIDATE:
      status = CPXXcallbackrejectcandidate (context, 1, nzcnt, &rhs,
                                            &sense, &beg,
                                            localdata->cutind,
                                            localdata->cutval);
      if ( status ) {
         fprintf (stderr,
                  "Error in CPXXcallbackrejectcandidate: status = %d\n",
                  status);
         goto TERMINATE;
      }
      break;
   case CPX_CALLBACKCONTEXT_RELAXATION:
      ...
      local = 0;
      status = CPXXcallbackaddusercuts (context, 1, nzcnt, &rhs, &sense, &beg,
                                        localdata->cutind,
                                        localdata->cutval,
                                        &purgeable, &local);
      if ( status ) {
         fprintf (stderr,
                  "Error in CPXXcallbackaddusercuts: status = %d\n", status);
         goto TERMINATE;
      }
      break;
   default:
