...
/* xglobalqpex1.c - Reading in and optimizing a (convex or non-convex) QP
 * problem with convex, first order or global optimizer.*/

/* To run this example, command line arguments are required.
   i.e.,   xglobalqpex1   filename   optimalitytarget
   where
...
main (int argc, char *argv[])
{
   ...
   switch (argv[2][0]) {
      case 'c':
         status = CPXXsetintparam (env, CPXPARAM_OptimalityTarget,
                                   CPX_OPTIMALITYTARGET_OPTIMALCONVEX);
      ...
      case 'g':
         ...
         status = CPXXqpopt (env, lp);
         if ( status ) {
            fprintf (stderr, "Failed to optimize nonconvex QP.\n");
            goto TERMINATE;
         }
         break;
      default:
   ...
   if ( solntype == CPX_BASIC_SOLN ) {
      cstat = (int *) malloc (cur_numcols*sizeof(*cstat));
      rstat = (int *) malloc (cur_numrows*sizeof(*rstat));
   ...
   x = (double *) malloc (cur_numcols*sizeof(*x));
   if ( x == NULL ) {
