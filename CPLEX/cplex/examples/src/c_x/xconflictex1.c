...
main(int argc, char **argv)
{
   ...
   lp = CPXXcreateprob(env, &status, "conflictex1");
   if ( status != 0 ) {
   ...
   nrows = CPXXgetnumrows (env, lp);
   nqconstrs = CPXXgetnumqconstrs (env, lp);
   nsos = CPXXgetnumsos (env, lp);
   nindconstrs = CPXXgetnumindconstrs (env, lp);
   npwl = CPXXgetnumpwl (env, lp);
   ...
   status = CPXXrefineconflictext (env, lp, grpcnt, grpcnt, grppref,
   ...
   {
      int solstat = CPXXgetstat (env, lp);
      fprintf (stdout, "Solution status: %s (%d)\n",
               CPXXgetstatstring (env, solstat, errbuf),
               solstat);
   }
   ...
   status = CPXXgetconflictext (env, lp, grpstat, 0, grpcnt - 1);
   ...
   {
      CPXDIM numBoundConf = 0;
      CPXDIM numConConf = 0;
      CPXDIM numQConConf = 0;
      CPXDIM numSosConf = 0;
      CPXDIM numIndConConf = 0;
      CPXDIM numPwlConf = 0;
   ...
   status = CPXXclpwrite (env, lp, CONFLICT_FILE);
