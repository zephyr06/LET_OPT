...
/* xmipex3.c - Entering and optimizing a MIP problem with SOS sets
...
main (void)
{
   ...
   status = CPXXwriteprob (env, lp, "mipex3.mps", NULL);
   if ( status ) {
   ...
   if (!status) CPXXsetintparam (env, CPXPARAM_Preprocessing_Aggregator,
                                 CPX_OFF);
   if (!status) CPXXsetcntparam (env, CPXPARAM_MIP_Interval, 1);
...
setsosandorder (CPXENVptr env, CPXLPptr lp)
{
   ...
   status = CPXXcopyorder (env, lp, 2, colindex, priority, direction);
   if ( status ) {
      fprintf (stderr, "CPXXcopyorder failed.\n");
      goto TERMINATE;
   }
   ...
   status = CPXXcopysos (env, lp, 1, 2, sostype, sosbeg,
                        sosind, sosref, NULL);
   if ( status ) {
      fprintf (stderr, "CPXXcopysos failed.\n");
      goto TERMINATE;
   }

   /* To assist in debugging, write the order to a file. */

   status = CPXXordwrite (env, lp, "mipex3.ord");
   if ( status ) {
      fprintf (stderr, "CPXXordwrite failed.\n");
      goto TERMINATE;
   }
