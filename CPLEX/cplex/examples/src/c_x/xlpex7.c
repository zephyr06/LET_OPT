...
/* xlpex7.c - Reading in and optimizing a problem.  Printing
             names with the answer.  This is a modification of
             lpex2.c  */

/* To run this example, command line arguments are required.
      xlpex7   filename   method
   where 
...
main (int argc, char *argv[])
{
   ...
   switch (argv[2][0]) {
      ...
      case 'b':
         method = CPX_ALG_BARRIER;
         status = CPXXsetintparam (env, CPXPARAM_SolutionType, CPX_NONBASIC_SOLN);
         if ( status ) {
            fprintf (stderr, 
   ...
   status = CPXXsetintparam (env, CPXPARAM_LPMethod, method);
   if ( status ) {
      fprintf (stderr, 
   ...
   /* Now get the column names for the problem.  First we determine how
      ...
      Then we call CPXXgetcolname() to get the actual names. */ 

   status = CPXXgetcolname (env, lp, NULL, NULL, 0, &surplus, 0,
                           cur_numcols-1);
   ...
   if ( cur_colnamespace > 0 ) {
      cur_colname      = malloc (sizeof(*cur_colname)*cur_numcols);
      cur_colnamestore = malloc (sizeof(*cur_colnamestore)*cur_colnamespace);
      if ( cur_colname      == NULL ||
      ...
      status = CPXXgetcolname (env, lp, cur_colname, cur_colnamestore, 
                              cur_colnamespace, &surplus, 0, cur_numcols-1);
      if ( status ) {
         fprintf (stderr, "CPXXgetcolname failed.\n");
         goto TERMINATE;
      }
   }
   else {
