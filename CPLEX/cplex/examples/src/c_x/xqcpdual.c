                       ...
                       double       zerotol)
{
   ...
   for (i = 0; i < numqs; ++i) {
      ...
      for (k = linbeg[i]; k < linbeg[i] + linnzcnt[i]; ++k) {
         grad[linind[k]] += linval[k];
         if ( fabs (x[linind[k]]) > zerotol )
            conetop = 0;
      }
...
main (void)
{
   ...
   for (i = 0; i < NUMQS; ++i) {
      ...
      /* CPXXaddqconstr adds just one quadratic constraint, therefore
	 the number of nonzeros in the linear part fits into a CPXDIM
	 value and so the third argument is of type CPXDIM. */
      status = CPXXaddqconstr (env, lp, (CPXDIM)(linend - linbeg[i]),
