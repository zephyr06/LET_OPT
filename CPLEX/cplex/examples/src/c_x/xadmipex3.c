      ...
      char *argv[])
{
   ...
   status = CPXXsetcntparam (env, CPXPARAM_MIP_Limits_Nodes, 10000);
   if ( status )  goto TERMINATE;

   /* Open a logfile */

   status = CPXXsetlogfilename (env, "xadmipex3.log", "a");
   if ( status )  goto TERMINATE;
               ...
               int          *useraction_p)
{
   ...
   *useraction_p = CPX_CALLBACK_DEFAULT;
   ...
   status = CPXXgetdblparam (env, CPXPARAM_MIP_Tolerances_Integrality, &epint);
