...
/* xadmipex4.c -- Solving noswot by adding cuts.
 *
 ...
 * branch-and-cut process, see xadmipex5.c.
 *
...
main (int argc, char *argv[])
{
   ...
   status = CPXXsetintparam (env, CPXPARAM_Preprocessing_Reformulations,
...
addcuts (CPXENVptr env, CPXLPptr lp)
{
   ...
   CPXNNZ cutbeg[] = {0, 2, 4, 6, 16, 26, 36, 46, 56};
   ...
   /* Use CPXXaddusercuts when the added constraints strengthen the
    * formulation but do not change the integer feasible region.
    * Use CPXXaddlazyconstraints when the added constraints remove part of
    ...
    * cases.
    */
   status = CPXXaddusercuts (env, lp, cuts, nz, cutrhs, 
   ...
   status = CPXXaddlazyconstraints (env, lp, cuts, nz, cutrhs, 
                                    cutsens, cutbeg, cutind, cutval, NULL);
   if ( status ) {
      fprintf (stderr, "Failed to add lazy constraints cuts.\n");
      goto TERMINATE;
   }
