...
/* xqcpex1.c - Entering and optimizing a quadratically constrained problem */

/* Bring in the CPLEX function declarations and the C library
   header file stdio.h with include of cplexx.h. */
...
static int
   setproblemdata (char **probname_p, CPXDIM *numcols_p, CPXDIM *numrows_p,
		   ...
		   CPXDIM **qconrow_p, CPXDIM **qconcol_p, double **qconval_p);
...
main (void)
{
   ...
   status = CPXXaddqconstr (env, lp, 0, NUMQCONNZ, 1.0, 'L',
   ...
   status = CPXXsolution (env, lp, &solstat, &objval, x, NULL, slack, NULL);
   if ( status ) {
   ...
   status = CPXXwriteprob (env, lp, "qcpex1.lp", NULL);
   if ( status ) {
		...
		CPXDIM **qconrow_p, CPXDIM **qconcol_p, double **qconval_p)
{
   ...
   CPXDIM   *zqconrow = NULL;
   CPXDIM   *zqconcol = NULL;
   ...
   zqmatind  = malloc (NUMQOBJNZ * sizeof(*zqmatind));
   zqmatval  = malloc (NUMQOBJNZ * sizeof(*zqmatval));
   zqconrow  = malloc (NUMQCONNZ * sizeof(*zqconrow));
   zqconcol  = malloc (NUMQCONNZ * sizeof(*zqconcol));
   zqconval  = malloc (NUMQCONNZ * sizeof(*zqconval));
