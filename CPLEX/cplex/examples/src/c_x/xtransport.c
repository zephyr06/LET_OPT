/* --------------------------------------------------------------------------
 * File: examples/src/cplexx/xtransport.c
 * Version 22.1.0
 * --------------------------------------------------------------------------
...
main (int argc, char **argv)
{
   ...
   lp = CPXXcreateprob (env, &status, "xtransport");
   ...
   status = CPXXwriteprob (env, lp, "xtransport.lp", NULL);
   if ( status ) {
      fprintf (stderr, "Failed to write LP to disk.\n");
      goto TERMINATE;
   }

   status = CPXXgetx (env, lp, x, 0, SUPPLY_X_DEMAND-1);
   if ( status ) {
               ...
               int convex)
{
   ...
   char const *names[SUPPLY_X_DEMAND];
    ...
    * recipient j */
   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      sprintf((char *)names[i], "x%d", i+1);
      cost[i] = 0.0;
   }
   status = CPXXnewcols (env, lp, SUPPLY_X_DEMAND, cost,
   ...
   for (i = 0; i < NUM_SUPPLY; i++) {
      ...
      status = CPXXaddrows (env, lp, 0, 1, NUM_DEMAND, &supply[i], "E",
   ...
   for (j = 0; j < NUM_DEMAND; j++) {
      ...
      status = CPXXaddrows (env, lp, 0, 1, NUM_SUPPLY, &demand[j], "E",
                            &zero, ind, val, NULL, NULL);
      if ( status )  goto TERMINATE;
   }
   ...
   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      sprintf((char *)names[i], "p%d", i+1);
   }

   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      status = CPXXaddpwl (env, lp, SUPPLY_X_DEMAND + i, i,
