...
/* xfixnet.c - Use indicator constraints to avoid numerical trouble in a
...
/* Define origin and destination nodes for each edge, as well as
   ...
   Note that by defining a fixed cost of 0 for each arch you just
   get a regular min-cost flow problem. */
static int const    orig[NUMEDGES]      = {0,0,1,1,2,2};
static int const    dest[NUMEDGES]      = {1,2,3,4,3,4};
static double const unitcost[NUMEDGES]  = {0,0,0,0,0,5};
static double const fixedcost[NUMEDGES] = {1,1,1,10,1,1};


/* Define demand (supply) at each node. */

static double const demand[NUMNODES] = {-1000001, 0, 0, 1000000, 1};
