...
static MULTIOBJINFO multiobjstats[] = {
   ...
   ROW (LONG_TYPE, CPX_MULTIOBJ_ITCNT),
   ROW (DOUBLE_TYPE, CPX_MULTIOBJ_OBJVAL),
   ROW (DOUBLE_TYPE, CPX_MULTIOBJ_BESTOBJVAL),
   ROW (LONG_TYPE, CPX_MULTIOBJ_NODECNT),
   ROW (LONG_TYPE, CPX_MULTIOBJ_NODELEFTCNT),
};

union multiobjval {
   int i;
   long long l; // FIXME: shouldn't this just be a long?
   double d;
};
...
static int
   ...
   populatebycolumn  (CPXENVptr env, CPXLPptr lp,
                      ...
                      int nnutr, double *nutrmin, double *nutrmax,
                      double **nutrper),
   useintvars        (CPXENVptr env, CPXLPptr lp, int nfoods),
   usemultiobj       (CPXENVptr env, CPXLPptr lp, int nfoods),
...
main (int argc, char **argv)
{
   ...
   if ( multiobj ) {
      ...
      for (i = 0; i < 2; ++i) {
         ps[i] = CPXparamsetcreate (env, &status);
         if ( ps[i] == NULL || status ) {
      ...
      status = CPXsetdblparam (env, CPXPARAM_DetTimeLimit, 60000.0);
      if ( status ) {
      ...
      status = CPXparamsetadddbl (env, ps[0], CPXPARAM_DetTimeLimit, 50000.0);
      if ( status ) {
         fprintf (stderr, "Failed to add parameter.\n");
         goto TERMINATE;
      }

      status = CPXparamsetadddbl (env, ps[1], CPXPARAM_DetTimeLimit, 25000.0);
      if ( status ) {
         fprintf (stderr, "Failed to add parameter.\n");
         goto TERMINATE;
      }
      ...
      status = CPXmultiobjopt (env, lp, (CPXCPARAMSETptr *)(&ps[0]));
   }
   else if ( intvars )
   ...
   buy = malloc (CPXgetnumcols (env, lp) * sizeof(*buy));
   if ( buy == NULL ) {
   ...
   status = CPXsolution (env, lp, &solstat, &objval, buy, NULL, NULL, NULL);
   if ( status ) {
   ...
   if ( multiobj ) {
      for (i = 0; i < CPXgetnumobjs (env, lp); ++i) {
         double obj;
         char objname[256];
         int surplus;

         status = CPXmultiobjgetobjval (env, lp, i, &obj);
         if ( status )  {
            fprintf (stderr, "Failed to obtain multiobj solution.\n");
            goto TERMINATE;
         }

         status = CPXmultiobjgetname (env, lp, i, objname, 256, &surplus);
         if ( status ) {
            fprintf (stderr, "Failed to get objective name.\n");
            goto TERMINATE;
         }
...
TERMINATE:
   ...
   for (i = 0; i < 2; ++i) {
      if ( ps[i] != NULL ) {
         status = CPXparamsetfree (env, &ps[i]);
         if ( status ) {
...
static int
populatebyrow (CPXENVptr env, CPXLPptr lp,
               int nfoods, double *cost, double *foodmin, double *foodmax,
               int nnutr, double *nutrmin, double *nutrmax,
               double **nutrper)
{
   int status = 0;
   ...
   status = CPXnewcols (env, lp, nfoods, cost, foodmin, foodmax, NULL, NULL);
...
useintvars (CPXENVptr env, CPXLPptr lp, int nfoods)
{
   ...
   status = CPXchgctype (env, lp, nfoods, ind, xctype);
   if ( status )  goto TERMINATE;
...
usemultiobj (CPXENVptr env, CPXLPptr lp, int nfoods)
{
   ...
   char *xctype = NULL;
   int i;
   int const nbefore = CPXgetnumcols (env, lp);
   int *indtype = NULL;
   int *indvar = NULL;
   ...
   status = CPXnewcols (env, lp, nfoods, NULL, lb, ub, xctype, NULL);
   if ( status )  goto TERMINATE;
   ...
   status = CPXaddindconstraints (env, lp, nfoods, indtype, indvar,
    ...
    * objective. */
   status = CPXsetnumobjs (env, lp, 2);
   if ( status )  goto TERMINATE;

   /* Retrieve the current objective (i.e., for the first objective). */
   status = CPXgetobj (env, lp, linval, 0, nfoods - 1);
   if ( status )  goto TERMINATE;
   ...
   status = CPXmultiobjsetobj (env, lp, 0, nfoods, linind, linval,
    ...
    * max(sum) as min(-sum). */
   status = CPXmultiobjsetobj (env, lp, 1, nfoods, indvar, linval,
                               CPX_NO_OFFSET_CHANGE,
                               -1.0, /* weight */
                               1,    /* priority */
                     ...
                     MULTIOBJINFOptr info)
{
   ...
   switch (info->type) {
   case INT_TYPE:
      status = CPXmultiobjgetintinfo (env, lp, subprob, &val.i, info->what);
      break;
   case LONG_TYPE:
      status = CPXmultiobjgetlonginfo (env, lp, subprob, &val.l, info->what);
      break;
   case DOUBLE_TYPE:
      status = CPXmultiobjgetdblinfo (env, lp, subprob, &val.d, info->what);
      break;
   default:
...
{
   int status = 0;
   const int num = CPXmultiobjgetnumsolves (env, lp);
   int i;
   ...
   for (i = 0; i < num; ++i) {
      ...
      status = CPXmultiobjgetintinfo (env, lp, i, &prio,
