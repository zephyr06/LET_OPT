...
/* multiobjex1.c - Reading in and optimizing a multiobjective problem;
 *                 uses cplexx.h */

/* To run this example, command line arguments are required.
       multiobjex1   filename paramset.prm
   where
   ...
   Example:
       multiobjex1  example.mps paramset.prm
 */
...
main (int argc, char *argv[])
{
   ...
   status = CPXmultiobjopt(env, lp, (CPXCPARAMSETptr *)paramsets);
   if ( status ) {
...
#define buffersize 50
      ...
      int priority;
      status = CPXmultiobjgetobjval (env, lp, i, &objval);
      if ( status ) {
         fprintf (stderr, "Failed to obtain objective value.\n");
         goto TERMINATE;
      }

      status = CPXmultiobjgetname (env, lp, i, oname, buffersize, &surplus);
      if ( surplus < 0 ) {
      ...
      status = CPXmultiobjgetobj (env, lp, i, NULL, 1, 0, NULL,
...
TERMINATE:
   ...
   if ( paramsets != NULL && paramsets[0] != NULL ) {
      status = CPXparamsetfree (env, &paramsets[0]);
      if ( status ) {
         fprintf (stderr, "CPXparamsetfree failed, error code %d.\n", status);
      }
   }
